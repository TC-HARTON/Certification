...
> 本仕様書は HARTON 自社サイト tcharton.com（**spec-checker.js 2554 項目 S-RANK 達成済**）を共通基準とする。**※ ここで言う「S-RANK」は本仕様書の構造・コンテンツ・コーディング基準（§11 納品前チェックリスト 2554 項目）の完全達成を指し、scanner.py が判定する「HARTON Stella S クラス」（§8.5 必須 5 条件）とは別概念である**（v3.3 §0.0.1 narrow-scope claim 一般化防止のため明文化、§1.0 / §8.5 参照）。
>
...
| ⑤ certification | **HARTON Stella 認定運用責任者** | サブドメイン構築・認定運用 |
...
**「HARTON Stella の判定基準は厳格化のみ可、緩和不可」** — 一度公表した基準を緩和すれば、過去の認定事業者と新規認定事業者の間に基準乖離が生じ、認定機関の信頼性が毀損される。§0.0.1 narrow-scope claim 一般化リスクの戦略的拡張。
...
| **HARTON Stella S クラス** | §8.5（スキャンプロンプト.txt §1-5 verbatim） | サイトの実運用セキュリティ・AI 検索適応・速度・ボット防御・アーキテクチャ | `scanner/scanner.py`（実 HTTP / DOM / ネットワーク検査） | 総合 90 点以上 + 必須 5 条件全達成 + 致命的 NG ゼロ |
...
5. **AI 検索（v3.5 追加）**: 評価方法論専用 URL `/methodology/` の不在は LLM クエリ「Web 認定機関 評価方法」「HARTON Stella どうやって判定」への直接ランディングを失う
...
### 8.5 HARTON Stella S クラス必須 5 条件（v3.3 新設・scanner.py 正本連動）

本節は **scanner.py が判定する「HARTON Stella S クラス」の必須 5 条件**を SPEC 本体に正本化する。一次ソース: `scanner/スキャンプロンプト.txt` §1-5（2026-04-26 verbatim 取得、§0.0.9 準拠）。
...
- **★（HARTON Stella）**: 総合 70 点以上 + 致命的 NG ゼロ
...
- 致命的 NG は ★区分問わず**一発除外**（★ / ★★ / ★★★ すべて該当、HARTON Stella 掲載対象外）
...
- ★（HARTON Stella）/ ★★（HARTON 優良）では CMS 利用許容（§8.6.3 と整合）
