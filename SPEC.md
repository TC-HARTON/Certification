...
6. **段階的公開の禁止**: §1.6 に従い、18 ページ未満での本番公開は許容しない
...
| 15 | `/contact/` | お問い合わせ | 必須 | marketing |
| 16 | `/legal/` | 特定商取引法表記 | 必須 | reading |
| 17 | `/privacy/` | プライバシーポリシー | 必須 | reading |
| 18 | `/news/` | お知らせ | 必須 | reading |
...
### 1.6 18ページ未満の運用は許容しない(v3.2 強制)

tcharton.com の本番運用では、本 §1.2 の 18 ページすべてが**最低限の必須ページ**である。「料金は後で」「FAQ は後で」等の段階的公開は **許容しない**。理由：

1. **E-E-A-T**: 料金・特商法・プライバシー等の不在は信頼性 FAIL
2. **GEO**: FAQ・事例の不在は LLM 引用率を著しく低下させる
3. **コンバージョン**: お問い合わせ・料金の不在は問い合わせ率を低下させる
4. **法令**: 特商法表記・プライバシーポリシーは法的必須

ローンチを早めるための「ページ削減」は SPEC v3.2 違反である。**18 ページ完成までローンチを延期**するのが正しい運用。
...
- GBP 未作成の場合は SPEC 違反（spec-checker FAIL）→ ★3 取得不可
...
- **S クラス取得条件**: 総合 90 点以上 + 判定可能な必須条件すべて達成 + 致命的 NG ゼロ（§8.6）
- **A クラス（HARTON 優良）**: 総合 80 点以上 + 必須条件 3 件以上達成
- **B クラス（HARTON Certified）**: 総合 70 点以上
- **保留扱い**: フォームなしサイトで条件 4、CWV 測定失敗時の条件 3 等
...
**位置付け**: ★区分（HARTON Certified ★3〜★5）問わず**一発除外**となる条件群。掲載自体を不可とする「機関の信頼性根幹」を守る最低条件。
...
- 致命的 NG 1 件以上検出 → `calculate_rating()` が**強制 D**（"Dクラス（致命的NG により非掲載）"）を返す（scanner.py Phase 9d / 行 2474-2479）
...
- 致命的 NG は ★区分問わず**一発除外**（★3〜★5 すべて該当、HARTON Certified 掲載対象外）
- §8.5 条件 5「SSG / Jamstack」は **★5 必須条件**であり、致命的 NG とは別概念
- ★3-★4 では CMS（WordPress 等）利用は**許容**、ただし**バージョン情報露出は禁止**（致命的 NG #4）
...
#### 8.9.1 必須要件（HARTON Certified ★5 条件）
...
- ★3（HARTON Certified）/ ★4（HARTON 優良）では CMS 利用許容（§8.6.3 と整合）
- ただし致命的 NG #4「CMS バージョン情報露出」は禁止
- ★5（HARTON S-Class）取得には SSG / Jamstack 必須
...
- scanner ④ の §8.9.3 判定ロジック拡張完了後、scanner で再判定 → 必須条件 5 達成確認 → ★5（S クラス）取得へ
