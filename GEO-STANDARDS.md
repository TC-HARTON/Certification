...
**バージョン:** 2.1（2026-04-25 / SPEC.md v3.1 整合・KDD 2024 以降の後続研究 (§8B) を統合・White Hat 原則・Multimodal GEO 認識追加）
**初版:** 2026-04-13
**改版:** 2026-04-16 v2.0（公的ソース実 URL リスト・llms.txt 対応）
...
## 8B. KDD 2024 以降の GEO 後続研究と White Hat 原則（v2.1 新設）

**典拠**: Awesome-GEO リポジトリ <https://github.com/DavidHuji/Awesome-GEO>（2026-04-25 取得）に掲載された 2024-2026 の主要論文を整理。本書 §3 の 9 戦略を一次基準としつつ、後続研究の知見を補強として認識する。

### 8B.1 後続主要論文一覧（取得日 2026-04-25）

| 年 | 論文 | arXiv | 重要度 |
|---|---|---|---|
| 2024 | Adversarial Search Engine Optimization for LLMs | arXiv:2406.18382 | 中（防御側知見） |
| 2024 | Manipulating Large Language Models to Increase Product Visibility | arXiv:2404.07981 | 高（攻撃手法の可視化） |
| 2024 | What Evidence Do Language Models Find Convincing? | arXiv:2402.11782 | 高（説得メカニズム） |
| 2024 | Ranking Manipulation for Conversational Search Engines | arXiv:2406.03589 | 中 |
| 2024 | GASLITeing The Retrieval | arXiv:2412.20953 | 中（埋込検索の脆弱性） |
| 2025 | **C-SEO Bench (NeurIPS D&B 2025)** | arXiv:2506.11097 | ★最高（公式ベンチマーク） |
| 2025 | White Hat Search Engine Optimization using LLMs | arXiv:2502.07315 | 高（倫理基準） |
| 2025 | StealthRank | arXiv:2504.05804 | 中（covert manipulation） |
| 2025 | Role-Augmented Intent-Driven GEO | arXiv:2508.11158 | 中 |
| 2026 | **Multimodal Generative Engine Optimization** | arXiv:2601.12263 | ★最高（画像/動画への拡張） |
| 2026 | IF-GEO（multi-query 衝突解決） | arXiv:2601.13938 | 中 |

### 8B.2 White Hat 原則（絶対法規・arXiv:2502.07315 整合）

GEO 9 戦略のうち #9 Keyword Stuffing は本仕様で禁止済（§3.1）。これに加え、2024-2025 の Adversarial GEO 研究（arXiv:2406.18382 / arXiv:2504.05804 StealthRank / arXiv:2412.20953 GASLITeing）が示す以下の手法も**絶対禁止**とする:

1. **プロンプト経由の不正な順位操作**: LLM の system prompt / context window を悪用して引用順位を操作する技法
2. **隠蔽型ランキング攻撃 (StealthRank)**: ユーザーには見えない形で LLM を誘導するスタイル
3. **埋込検索の Adversarial 攻撃 (GASLITe)**: 埋込ベクトル空間を欺く coordinated content
4. **Adversarial SEO**: 競合サイトを LLM 引用から除外する目的の操作

本仕様は **White Hat GEO（=正攻法のみ）** を採用し、上記手法を発見した場合は SPEC §0.0.1 の背任に準じて即時撤去・user 申告する。

### 8B.3 Multimodal GEO（arXiv:2601.12263 整合）

画像・動画コンテンツも生成エンジン引用の対象になりつつある。本仕様は SPEC §6.3 画像仕様に対し、以下を **追加要件**として認識する:

- **画像 alt の二重役割**: 「視覚障害者向け説明」だけでなく「Multimodal LLM 引用用 metadata」も担う
- **装飾画像でない場合**: alt は具体数値・固有名詞・文脈情報を含める（曖昧な「写真」「画像」禁止）
- **`<figure>` + `<figcaption>` セマンティック紐付け**: 画像と説明を機械可読に対応させる
- **画像近接の数値・引用**: G-2 / G-3（公的リンク・数値）の近傍に画像を置くと Multimodal LLM の引用率向上が期待される（arXiv:2601.12263 推定値、本仕様での実測は v3.x で実施予定）

### 8B.4 C-SEO Bench（arXiv:2506.11097, NeurIPS 2025）の活用方針

Conversational SEO の公式ベンチマーク。本仕様の GEO 検証項目（§8）に将来的に C-SEO Bench スコア取得を組込む候補:

- **v3.x 検討課題**: spec-checker に C-SEO Bench スコアプローブを追加できるか
- **データセット**: 公開コードと dataset を活用し、HARTON サイトと site-builder 生成サイトの C-SEO Bench スコアを定期測定
- **判定基準**: ベンチマーク上位 N% を S-RANK 拡張要件として採用するかは v3.x で議論

### 8B.5 後続研究を踏まえた本仕様の方針

1. KDD 2024 (§3 の 9 戦略) を**一次基準**として継続採用（数値の信頼性・査読済み）
2. 2024-2026 の後続研究は **White Hat の追認**および **Multimodal 拡張** として参照
3. 本仕様 v2.1 では「禁止行為の明確化」までを反映。検証項目への組込みは v2.2 以降で段階導入
