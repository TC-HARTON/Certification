 ...
 * + 納品前チェックリスト
 * + GEO/LLMO (G-1〜G-6)
 * + 動的ページ（regions/industries/businesses/rankings）は glob で自動列挙
 *
 * 使い方:
...
const STATIC_TARGETS = [
  ...
  'methodology/technical/index.html',
  'methodology/security/index.html',
  'methodology/ai-search/index.html',
  'methodology/business-impact/index.html',

  // 動線（4）
  'apply/index.html',
  'improvement-guide/index.html',
  'press/index.html',
  'opt-out/index.html',
  ...
  'news/shizuoka-industry-report-2026-q2/index.html',
];

// 動的ページの glob ルート（存在すれば自動列挙）
const DYNAMIC_GLOB_ROOTS = ['regions', 'industries', 'businesses', 'rankings'];

function collectDynamicTargets() {
  const out = [];
  for (const r of DYNAMIC_GLOB_ROOTS) {
...
const TARGET_FILES = [...STATIC_TARGETS, ...collectDynamicTargets()];

// ページ種別 → 検証強度
// full    : TOP（高度 JSON-LD: Organization/WebSite/FAQPage/BreadcrumbList/Person）
// hub     : methodology メイン / 業種・地域インデックス（3 種）
// subpage : 階層下位ページ（BreadcrumbList のみ必須）
...
const PAGE_TYPE = {
  ...
  'methodology/technical/index.html': 'subpage',
  'methodology/security/index.html': 'subpage',
  'methodology/ai-search/index.html': 'subpage',
  'methodology/business-impact/index.html': 'subpage',

  'apply/index.html': 'subpage',
  'improvement-guide/index.html': 'subpage',
  'press/index.html': 'subpage',
  'opt-out/index.html': 'minimal',
  ...
  '404.html': 'minimal',
  'thanks.html': 'minimal',
  'news/shizuoka-industry-report-2026-q2/index.html': 'subpage',
};

// 動的ページの type は path から推定
function getPageType(relPath) {
  if (PAGE_TYPE[relPath]) return PAGE_TYPE[relPath];
  if (/^businesses\/[^/]+\/index\.html$/.test(relPath)) return 'business';
  if (/^businesses\/[^/]+\/badge\/index\.html$/.test(relPath)) return 'minimal';
  if (/^regions\//.test(relPath)) return 'hub';
  if (/^industries\//.test(relPath)) return 'hub';
  if (/^rankings\//.test(relPath)) return 'subpage';
  return 'subpage';
}
...
function c11_3_eeat() {
  ...
  // 1. about/index.html 存在（certification は about/ で代表略歴 + E-E-A-T 担保 / MASTER-PLAN §4.1）
  const pp = path.join(ROOT, 'about', 'index.html');
  r.push(fs.existsSync(pp)
    ? PASS('11.3-about', S, 'about ページ（E-E-A-T）')
    : FAIL('11.3-about', S, 'about ページ', 'about/index.html 不存在'));

  // 2. 一次情報（自動検出は困難→about ページに内容があるかで簡易チェック）
  if (fs.existsSync(pp)) {
    const c = fs.readFileSync(pp, 'utf-8');
    const bodyLen = (body(c) || c).replace(/<[^>]+>/g, '').trim().length;
    r.push(bodyLen > 200
      ? PASS('11.3-1st', S, '一次情報(about 内容)', `${bodyLen}文字`)
      : WARN('11.3-1st', S, '一次情報', `about 内容が少ない(${bodyLen}文字)`));
  } else {
    r.push(FAIL('11.3-1st', S, '一次情報', 'about ページ不存在'));
  }

  // 3. 孤立ページなし — MASTER-PLAN §4.1 22 ページ設計の主要導線
  const mainHtml = fs.existsSync(path.join(ROOT, 'index.html'))
    ? fs.readFileSync(path.join(ROOT, 'index.html'), 'utf-8') : '';
  const pages = [
    'about/', 'methodology/',
    'apply/', 'improvement-guide/', 'press/',
